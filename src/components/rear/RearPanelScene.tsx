    ...
    useEffect(() => {
      ...
      if (coverOpen) {
        ...
        if (topCoverRef.current) {
          tl.to(
            topCoverRef.current.position,
            { y: 0.8, duration: 0.7, ease: "power2.out" },
            0,
          );
          tl.to(
            topCoverRef.current.rotation,
            { x: -0.35, duration: 0.7, ease: "power2.out" },
            0,
          );
        }
        ...
        if (nmCoverRef.current) {
          tl.to(
            nmCoverRef.current.position,
            { y: 0.7, z: 0.3, duration: 0.65, ease: "power2.out" },
            0.1,
          );
          tl.to(
            nmCoverRef.current.rotation,
            { x: -0.6, duration: 0.65, ease: "power2.out" },
            0.1,
          );
        }
      } else {
        if (topCoverRef.current) {
          tl.to(
            topCoverRef.current.position,
            { y: PANEL_H / 2 + 0.025, duration: 0.6, ease: "power2.inOut" },
            0,
          );
          tl.to(
            topCoverRef.current.rotation,
        ...
        if (nmCoverRef.current) {
          tl.to(
            nmCoverRef.current.position,
            { y: 0, z: 0, duration: 0.55, ease: "power2.inOut" },
            0,
          );
          tl.to(
            nmCoverRef.current.rotation,
    ...
    return (
      <group>
        ...
        {Array.from({ length: 18 }).map((_, i) => (
          <mesh
            key={`vent-${i}`}
            position={[-1.8 + i * 0.22, -PANEL_H / 2 - 0.015, -0.01]}
            castShadow={false}
          >
