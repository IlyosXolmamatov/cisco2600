    ...
    >
      ...
      {Array.from({ length: 7 }).map((_, i) => {
        ...
        return (
          <mesh
            key={`blade-${i}`}
            position={[Math.sin(angle) * 0.13, Math.cos(angle) * 0.13, 0.035]}
            rotation={[0, 0, angle + 0.4]}
          >
            <boxGeometry args={[0.055, 0.2, 0.007]} />
            <meshStandardMaterial
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: 10 }).map((_, i) => (
        <mesh key={`trace-h-${i}`} position={[0, 0.031, -0.5 + i * 0.11]}>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <mesh key={`trace-v-${i}`} position={[-0.6 + i * 0.18, 0.031, 0]}>
      ...
      {[-0.42, -0.14, 0.14, 0.42].map((x, i) => (
        <mesh key={i} position={[x, 0.062, 0.59]}>
      ...
      {[-0.55, -0.27, 0.01, 0.29, 0.57].map((x, i) => (
        <mesh key={i} position={[x, 0.108, 0.59]}>
      ...
      {Array.from({ length: 28 }).map((_, i) => (
        <mesh key={`pin-${i}`} position={[0, -0.03, -0.52 + i * 0.037]}>
...
export function PSU({ position, onSelect, selectedComponent }: PSUProps) {
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: 10 }).map((_, i) => (
        <mesh key={`vent-top-${i}`} position={[-0.3 + i * 0.065, 0.162, 0.1]}>
      ...
      <mesh position={[0, 0.08, 0.392]}>
        <boxGeometry args={[0.82, 0.2, 0.012]} />
        <meshStandardMaterial
          color="#d8d8cc"
          metalness={0.05}
          roughness={0.88}
      ...
      <mesh position={[0, 0, -0.392]}>
        <boxGeometry args={[0.26, 0.2, 0.012]} />
  ...
  return (
    ...
    >
      ...
      <mesh position={[0, 0, -0.34]}>
        <boxGeometry args={[0.86, 0.245, 0.01]} />
        <meshStandardMaterial
          color="#444455"
          metalness={0.85}
          roughness={0.2}
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <mesh key={`trace-${i}`} position={[0, 0.022, -0.2 + i * 0.1]}>
      ...
      {Array.from({ length: 24 }).map((_, i) => (
        <mesh key={`pin-${i}`} position={[-0.33 + i * 0.028, -0.038, 0]}>
          <boxGeometry args={[0.018, 0.02, 0.036]} />
          <meshStandardMaterial
            color="#d4aa20"
            metalness={0.98}
            roughness={0.03}
...
/* ═══════════════════════════════════════════════════════
   DRAM — Memory Module (SIMM/DIMM Card)
   ═══════════════════════════════════════════════════════ */
interface DRAMProps {
  position: [number, number, number];
  slotIndex: number;
    ...
    <group position={position} name="dram-module">
      ...
      >
        ...
        <meshStandardMaterial
          ...
          metalness={0.08}
          roughness={0.65}
          emissive={hovered || isSelected ? "#0d6620" : "#000000"}
          emissiveIntensity={hovered ? 0.25 : isSelected ? 0.4 : 0}
        />
      </mesh>
      ...
      {(
        ...
        <mesh key={`chip-${i}`} position={[x, y, z]} castShadow>
          ...
          <meshStandardMaterial
            ...
            roughness={0.4}
          />
          {/* Chip markings (text representation) */}
          <mesh position={[0, 0.045, 0]}>
            <boxGeometry args={[0.3, 0.002, 0.15]} />
